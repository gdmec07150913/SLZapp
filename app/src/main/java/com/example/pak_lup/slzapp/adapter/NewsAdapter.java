...
public class NewsAdapter extends BaseAdapter {
    private List<ItemBean_news> mList;//数据源
    private LayoutInflater mInflater;//布局装载器对象

    public NewsAdapter(Context context, List<ItemBean_news> list) {
        mList = list;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            convertView = mInflater.inflate(R.layout.itemmessage_news, null);

            //对viewHolder的属性进行赋值
            viewHolder.imageView = (ImageView) convertView.findViewById(R.id.roundImageView);
            viewHolder.name = (TextView) convertView.findViewById(R.id.name_xiaoxi);
        ...
        // 取出bean对象
        ItemBean_news bean = mList.get(position);

        // 设置控件的数据
        viewHolder.imageView.setImageResource(bean.itemImageResId);
        viewHolder.name.setText(bean.itemName);
        viewHolder.message.setText(bean.itemMessage);
